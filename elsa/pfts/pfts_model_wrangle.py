...
#core_filtered["aerobically_active"] = np.where((core_filtered['heacta'] == 1) | (core_filtered['heacta'] == 1), 1, 0)
core_filtered["regular_alcohol"] = np.where((core_filtered['scako'] <= 3) & (core_filtered['scako'] >= 1), 1, 0)
core_filtered["taking_beta_blocker"] = np.where(core_filtered['HeBetb'] == 1, 1, 0) 
#core_filtered["married"] = np.where(core_filtered['couple'] == 1, 1, 0)

core_final = core_filtered[["idauniq","current_smoker","aerobically_active","regular_alcohol", "taking_beta_blocker"]]
...
common_final = common_filtered[["idauniq", "anxiety_stress"]]
...
nurse_filtered.loc[:, "mean_pulse"] = (nurse_filtered.loc[:, "pulse1"] + nurse_filtered.loc[:, "pulse2"] + nurse_filtered.loc[:, "pulse3"]) / 3
...
nurse_final = nurse_filtered[["idauniq", "sex", "confage", "mean_pulse", "htfvc", "htfev", "htpf", "mean_sys", "mean_dias", "has_dyslipidaemia"]]
...
#derived_filtered["has_diabetes"] = np.where(derived_filtered["hedacdi"] == 1, 1, 0)
derived_filtered["has_diabetes"] = np.where(derived_filtered["hedbts"] != 0, 1, 0)
#derived_filtered["has_hypertension"] = np.where(derived_filtered["hedacbp"] != 0, 1, 0)
derived_filtered["has_hypertension"] = np.where(derived_filtered["hedimdi"] != 0, 1, 0)
#derived_filtered["has_arthritis"] = np.where(derived_filtered["hedbdar"] == 1, 1, 0)
derived_filtered["has_arthritis"] = np.where(derived_filtered["hedibar"] != 0, 1, 0)
#derived_filtered["has_asthma"] = np.where(derived_filtered["hedbdas"] == 1, 1, 0)
derived_filtered["has_asthma"] = np.where(derived_filtered["hedibas"] != 0, 1, 0)
#derived_filtered["has_stroke"] = np.where(derived_filtered["hedacst"] == 1, 1, 0)
derived_filtered["has_stroke"] = np.where(derived_filtered["hedimst"] != 0, 1, 0)
#derived_filtered["has_mi"] = np.where(derived_filtered["hedacmi"] == 1, 1, 0)
derived_filtered["has_mi"] = np.where(derived_filtered["hedimmi"] != 0, 1, 0)
#derived_filtered["has_dementia"] = np.where((derived_filtered["hedbdad"] == 1) | (derived_filtered["hedbdde"] == 1), 1, 0)
derived_filtered["has_dementia"] = np.where((derived_filtered["hedibde"] != 0) | (derived_filtered["hedibad"] != 0), 1, 0)
derived_filtered["has_arrhythmia"] = np.where((derived_filtered["hedimar"] != 0), 1, 0)
derived_filtered["has_chronic_lung_disease"] = np.where((derived_filtered["hediblu"] != 0), 1, 0)
#derived_filtered["degree_educated"] = np.where(derived_filtered["w2edqual"] == 1, 1, 0)

derived_final = derived_filtered[["idauniq", "has_hypertension", "has_diabetes", "has_arthritis", "has_asthma", "has_chronic_lung_disease", "has_stroke", "has_mi", "has_dementia", "has_arrhythmia"]]

df_final = reduce(lambda left,right: pd.merge(left,right,on='idauniq'), [core_final, common_final, nurse_final, derived_final])

df_final.drop(columns=["idauniq"], inplace=True)

#print(df_final["mean_pulse"].describe())

df_final.to_csv('elsa/pfts/pft_model_wrangle.csv', index=False)